...
public class frmTrabajadorPru extends JFrame implements ActionListener, MouseListener, KeyListener, WindowListener {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					frmTrabajadorPru frame = new frmTrabajadorPru();
	...
	public frmTrabajadorPru() {
		...
		lblMantenimientoTrabajador.setForeground(new Color(255, 255, 255));
		...
		tblTabla.setGridColor(new Color(255, 0, 0));
		tblTabla.setForeground(new Color(0, 0, 0));
		tblTabla.setBackground(new Color(255, 255, 255));
		...
		try {
			...
			if(nombre.trim().length()==0) {
				...
				if(t == null) {
				    ...
				    ArrayList<Distrito> lisdi= trabajdorDao.listDis();
				    
				    int codydis = 0;
				    for (Distrito d : lisdi) {
						if(d.getDescr().equals(codDis)) {
							codydis= d.getCodigo();
							break;
						}
					}
				    bean.setCodDistrito(codydis);
